...
from bots.model import EYStream
...
class ChatEventType(DynamicStrEnum):
    ...
    GW_MESSAGE_DELTA = "gw.message.delta"
    ...
    GW_MESSAGE_COMPLETED = "gw.message.completed"
    ...
    # 此事件用于标识对话失败。
    GW_CHAT_FAILED = "conversation.chat.failed"
...
def _chat_stream_handler(data: Dict, raw_response: httpx.Response) -> Optional[ChatEvent]:
    ...
    if not event_data and event==ChatEventType.GW_MESSAGE_DELTA:
        return None
    if event == ChatEventType.DONE:
        ...
        ChatEventType.GW_MESSAGE_DELTA,
        ChatEventType.GW_MESSAGE_COMPLETED,
        ChatEventType.GW_CHAT_FAILED,
    ]:
        event = ChatEvent(event=event, message=event_data)
        ...
        event = ChatEvent(event=ChatEventType.UNKNOWN, message=event_data)
        event._raw_response = raw_response
        return event
...
class ChatClient(object):
    ...
    def messages(
        ...
        if self._messages is None:
            from cozepy.chat.message import ChatMessagesClient
...
class AsyncChatClient(object):
    ...
    def messages(
        ...
        if self._messages is None:
            from cozepy.chat.message import AsyncChatMessagesClient
